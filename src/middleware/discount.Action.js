...
const updateDiscount = async (idDiscount, idUser) => {
    ...
    try {
        ...
        if (count.length > 0) {
            await db.query(`update discount set quantity=(quantity-1) where discount_id=${idDiscount};`);
            await db.query(`delete from discount_user where dsc_id=${idDiscount} and user_id=${idUser}`);
